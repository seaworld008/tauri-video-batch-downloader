...
export function useAutoSync(options: UseAutoSyncOptions = {}) {
      ...
      try {
        ...
        if (enablePerformanceMonitoring) {
          ...
          setPerformanceMetrics(prev => ({
            ...
            averageResponseTime:
              (prev.averageResponseTime * prev.syncCount + syncDuration) / (prev.syncCount + 1),
  ...
  const updateSyncInterval = useCallback(() => {
    ...
    const newInterval = isPageVisible ? intervalMs : intervalMs * hiddenPageMultiplier;

    if (newInterval !== currentIntervalMs) {
      setCurrentIntervalMs(newInterval);
      console.log(
        `🧠 智能调度: 页面${isPageVisible ? '可见' : '隐藏'}，同步间隔调整为 ${newInterval}ms`
      );
    }

...
export const TaskItem: React.FC<TaskItemProps> = ({ task, isVirtualized = false }) => {
        ...
        <div className="grid grid-cols-12 gap-4 items-center">
          ...
          <div className="col-span-4">
            <div className="flex flex-col">
              ...
              {task.error_message && (
                <div className="text-xs text-red-600 dark:text-red-400 mt-1 truncate" title={task.error_message}>
          ...
          <div className="grid grid-cols-2 gap-6 mt-4 text-sm">
            <div>
              ...
              <div className="space-y-1 text-gray-600 dark:text-gray-400">
                <div><span className="font-medium">URL:</span> {task.url}</div>
                <div><span className="font-medium">输出路径:</span> {task.output_path}</div>
                {task.downloader_type && (
                  <div><span className="font-medium">下载器:</span> {task.downloader_type}</div>
                )}
                {task.eta && task.status === 'downloading' && (
                  <div><span className="font-medium">预计剩余:</span> {formatDuration(task.eta)}</div>
                )}
              </div>
            </div>
            <div>
              ...
              <div className="space-y-1 text-gray-600 dark:text-gray-400">
                <div><span className="font-medium">创建时间:</span> {formatDate(task.created_at)}</div>
                <div><span className="font-medium">更新时间:</span> {formatDate(task.updated_at)}</div>
                <div><span className="font-medium">任务ID:</span> {task.id}</div>
              </div>
            </div>
          </div>
        </div>

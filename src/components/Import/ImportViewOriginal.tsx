...
export const ImportView: React.FC<ImportViewProps> = () => {
  ...
  const previewImportData = async (filePath: string) => {
    ...
    try {
      ...
      preview.headers.forEach((header, index) => {
        ...
        // 匹配Go版本的标准字段名
        if (headerLower === 'record_url' || headerLower.includes('record_url') || 
            headerLower.includes('视频链接') || headerLower.includes('下载链接')) {
          defaultMapping[header] = 'record_url';
        } else if (headerLower === 'zl_id' || headerLower.includes('专栏id') || 
                   headerLower.includes('zl_id') || headerLower.includes('专栏标识')) {
          defaultMapping[header] = 'zl_id';
        } else if (headerLower === 'zl_name' || headerLower.includes('专栏名称') || 
                   headerLower.includes('zl_name')) {
          defaultMapping[header] = 'zl_name';
        } else if (headerLower === 'kc_id' || headerLower.includes('课程id') || 
                   headerLower.includes('kc_id') || headerLower.includes('课程标识')) {
          defaultMapping[header] = 'kc_id';
        } else if (headerLower === 'kc_name' || headerLower.includes('课程名称') || 
        ...
        } else if (headerLower.includes('name') || headerLower.includes('名称') || 
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      notify.success(
        '导入成功', 
        `成功导入 ${importedData.length} 个下载任务`
      );

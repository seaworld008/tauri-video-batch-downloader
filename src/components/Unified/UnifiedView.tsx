...
export const UnifiedView: React.FC = () => {
    ...
    return (
        <div className="h-screen flex flex-col bg-gray-50 dark:bg-gray-900 overflow-hidden relative">
            ...
            {/* Header Bar */}
            <header className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 h-14 flex items-center justify-between px-4 shadow-sm z-20 shrink-0">
            ...
            <main className="flex-1 flex flex-col overflow-hidden relative z-0">
                ...
                <div className={`bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm shrink-0 transition-all duration-300 ease-in-out ${isInputCollapsed ? 'max-h-12' : 'max-h-[500px]'}`}>
                    <div className="max-w-7xl mx-auto flex flex-col h-full">
                        ...
                        <div className="flex items-center justify-between border-b border-gray-100 dark:border-gray-700 px-4 bg-gray-50/50 dark:bg-gray-800/50">
                            <div className="flex gap-1">
                                <button
                                    onClick={() => { setActiveTab('manual'); setIsInputCollapsed(false); }}
                                    className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'manual' && !isInputCollapsed
                                ...
                                <button
                                    onClick={() => { setActiveTab('file'); setIsInputCollapsed(false); }}
                                    className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'file' && !isInputCollapsed
                                        ? 'border-blue-500 text-blue-600 dark:text-blue-400'
                                        : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400'
                                        }`}
                                >
                            ...
                            >
                                {isInputCollapsed ? <ChevronDownIcon className="w-4 h-4" /> : <ChevronUpIcon className="w-4 h-4" />}
                            </button>
                        </div>

                        {/* Panel Content */}
                        <div className={`overflow-hidden transition-all duration-300 ${isInputCollapsed ? 'opacity-0 h-0' : 'opacity-100 p-6'}`}>
                            {activeTab === 'manual' && <ManualInputPanel />}
                            {activeTab === 'file' && <FileImportPanel onImportSuccess={() => setIsInputCollapsed(true)} />}
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-hidden bg-gray-50 dark:bg-gray-900 px-4 pb-0 relative">
                    <div className="h-full max-w-7xl mx-auto border-x border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm overflow-hidden flex flex-col">
                        {tasks.length === 0 ? (
            ...
            {/* Drawer */}
            <div className={`fixed inset-y-0 right-0 w-full max-w-md bg-white dark:bg-gray-900 shadow-2xl z-50 transform transition-transform duration-300 ease-in-out ${showSettings ? 'translate-x-0' : 'translate-x-full'}`}>
            ...
            {/* About Modal */}
            {showAbout && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={() => setShowAbout(false)}>
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-sm w-full p-8 text-center transform transition-all scale-100" onClick={e => e.stopPropagation()}>
                        ...
                        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Video Downloader Pro</h2>
                        ...
                        <p className="text-gray-600 dark:text-gray-300 mb-8 text-sm leading-relaxed">
                            专业的批量视频下载工具<br />支持多线程下载、自动嗅探和断点续传
                        </p>
                        <button

...
export const FileImportPanel: React.FC<FileImportPanelProps> = ({ onImportSuccess }) => {
  ...
  const executeImport = useCallback(async () => {
    ...
    if (!canProceedWithImport(importPreview.headers, fieldMapping) || !backendFieldMapping.video_url) {
    ...
    try {
      ...
      const effectiveSnapshot = newTaskIds.length > 0
        ? updatedTasks.filter(t => newTaskIds.includes(t.id))
        : tasksToAdd; // Fallback if sync failed

      recordRecentImport(newTaskIds.length > 0 ? newTaskIds : tasksToAdd.map(t => t.id), effectiveSnapshot);

      notify.success(`成功导入 ${validRows.length} 个下载任务`);
  ...
  }, [addTasks, defaultOutputDirFromConfig, fieldMapping, importPreview, outputDir, refreshTasks, selectedFile, setFilterStatus, recordRecentImport, setSearchQuery, setSortBy, tasks, onImportSuccess]);
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                {importPreview.headers.map((header) => (
                  <div key={header} className="flex items-center gap-2 bg-gray-50 dark:bg-gray-700/50 p-2 rounded-lg">
                    <label className="text-sm text-gray-500 w-1/3 truncate" title={header}>{header}</label>
                    <select

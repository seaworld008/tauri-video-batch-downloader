...
export const ImportArea: React.FC<ImportAreaProps> = ({ className = '' }) => {
  ...
  const executeImport = useCallback(
    async (preview: ImportPreview, filePath: string, mapping: Record<string, string>) => {
      ...
      try {
        ...
        const addedTasks = await addTasks(tasks);
        const resolvedTasks = addedTasks.length > 0 ? addedTasks : tasks;
        ...
        useDownloadStore.setState({
          selectedTasks: newTaskIds.length > 0 ? newTaskIds : fallbackIds,
        });
  ...
  if (!selectedFile) {
    return (
      <div
        className={`bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-800 dark:to-gray-900 rounded-xl border-2 border-dashed border-blue-300 dark:border-gray-600 transition-all duration-300 hover:border-blue-400 dark:hover:border-gray-500 ${className}`}
      >
        <div className='p-6 text-center'>
          ...
          <div className='mt-4 grid grid-cols-2 gap-2 max-w-md mx-auto text-left'>
            {['🚀 智能字段识别', '🔄 自动编码检测', '📊 实时数据预览', '⚙️ 灵活映射配置'].map(
              feature => (
                <div
                  key={feature}
          ...
          <div className='flex items-center space-x-3'>
            ...
            <div>
              ...
              <p className='text-sm text-gray-500 dark:text-gray-400'>
                {importPreview
                  ? `${importPreview.total_rows} 行数据 • ${importPreview.encoding} 编码`
                  : '正在解析...'}
              </p>
            </div>
          </div>
                ...
                {importPreview.rows.slice(0, 3).map((row, rowIndex) => (
                  <tr key={rowIndex} className='hover:bg-gray-50 dark:hover:bg-gray-700'>
                    {row.map((cell, cellIndex) => (
                      <td
                        key={cellIndex}
                        className='px-3 py-2 text-sm text-gray-900 dark:text-gray-300 max-w-xs truncate'
                      >
                        {cell}
                      </td>
                    ))}
